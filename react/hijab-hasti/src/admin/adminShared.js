...
import { dataService } from './data/dataService.js';
    ...
    function daysUntil(jalaliDate) {
        return dataService.daysBetween(dataService.today(), jalaliDate);
    }
    ...
    var NAV = [
        {
            group: 'مرور کلی', items: [
                { key: 'dashboard', label: 'داشبورد', href: '/admin', icon: 'dashboard' }
            ]
        },
        {
            group: 'فروش', items: [
                { key: 'orders', label: 'سفارش‌ها', href: '/admin/orders', icon: 'cart', badge: 'pendingOrders' },
                { key: 'customers', label: 'مشتریان', href: '/admin/customers', icon: 'users' },
                { key: 'discounts', label: 'تخفیف و کمپین', href: '/admin/discounts', icon: 'percent' }
            ]
        },
        {
            group: 'کاتالوگ', items: [
                { key: 'products', label: 'محصولات', href: '/admin/products', icon: 'box' },
                { key: 'categories', label: 'دسته‌بندی و کالکشن', href: '/admin/categories', icon: 'layers' },
                { key: 'inventory', label: 'انبار', href: '/admin/inventory', icon: 'warehouse', badge: 'lowStock' }
            ]
        },
        {
            group: 'محتوا و ارتباط', items: [
                { key: 'blog', label: 'بلاگ', href: '/admin/blog', icon: 'article' },
                { key: 'reviews', label: 'نظرات', href: '/admin/reviews', icon: 'star', badge: 'pendingReviews' },
                { key: 'messages', label: 'پیام‌ها و تیکت', href: '/admin/messages', icon: 'inbox', badge: 'openTickets' }
            ]
        },
        {
            group: 'مدیریت', items: [
                { key: 'reports', label: 'گزارش‌ها', href: '/admin/reports', icon: 'chart' },
                { key: 'settings', label: 'تنظیمات فروشگاه', href: '/admin/settings', icon: 'settings' },
                { key: 'users', label: 'کاربران و دسترسی', href: '/admin/users', icon: 'shield' }
            ]
        }
    ];
    ...
    */
    ...
    async function renderAdminShell(activeKey) {
        ...
        var topbar = el(
            ...
            '<a class="icon-btn" href="/" target="_blank" rel="noopener" title="مشاهده سایت" aria-label="مشاهده سایت فروشگاه">' + icon('external') + '</a>' +
            ...
            '<a class="dropdown__item" href="/admin/users">' + icon('shield') + '<span>کاربران و دسترسی‌ها</span></a>' +
            '<a class="dropdown__item" href="/admin/settings">' + icon('settings') + '<span>تنظیمات فروشگاه</span></a>' +
            '<div class="dropdown__sep"></div>' +
            '<a class="dropdown__item dropdown__item--danger" href="/admin/login" id="logoutLink">' + icon('logout') + '<span>خروج از حساب</span></a>' +
            '</div>' +
            '</div>' +
    ...
    function bindShellEvents() {
        ...
        if (input && results) {
            input.addEventListener('input', debounce(async function () {
                ...
                var data = await dataService.globalSearch(q);
    ...
    function renderSearchResults(data) {
        ...
        if (data.products.length) {
            ...
            data.products.forEach(function (p) {
                html += '<a href="/admin/products/' + encodeURIComponent(p.id) + '">' + icon('box') +
        ...
        if (data.orders.length) {
            ...
            data.orders.forEach(function (o) {
                html += '<a href="/admin/orders/' + encodeURIComponent(o.orderNumber) + '">' + icon('cart') +
        ...
        if (data.customers.length) {
            ...
            data.customers.forEach(function (c) {
                html += '<a href="/admin/customers/' + encodeURIComponent(c.id) + '">' + icon('user') +
    ...
    async function loadNotifications() {
        ...
        if (!list) return;
        var items = await dataService.getNotifications();
    ...
    async function loadSidebarBadges() {
        var stats = await dataService.getDashboardStats();
        var map = {
